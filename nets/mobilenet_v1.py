...
def deeper_mobile_net_v1(inputs,
                 num_classes,
                 dropout_keep_prob=0.999,
                 is_training=True,
                 min_depth=8,
...
# ]
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
                        is_training=is_training):
      last_conv = len(deeper_conv_defs) - 1
      from nets import unet
      unet_out = unet.unet(inputs)
      net, end_points = mobilenet_v1_base(unet_out, scope=scope,
                                          min_depth=min_depth,
                                          depth_multiplier=depth_multiplier,
                                          conv_defs=conv_defs)
                                          # scope=scope,
                                          # min_depth=min_depth,
                                          # depth_multiplier=depth_multiplier,
                                          # # conv_defs=deeper_conv_defs, 
                                          # final_endpoint=('Conv2d_%d_pointwise' %last_conv))
      with tf.variable_scope('Logits'):
        kernel_size = _reduced_kernel_size_for_small_input(net, [7, 7])
        net = slim.avg_pool2d(net, kernel_size, padding='VALID',
                              scope='AvgPool_1a')
...
mobilenet_v1_110 = wrapped_partial(mobilenet_v1, depth_multiplier=1.1)
