...
# =============================================================================
"""Joint Anscombe (ISP) + MobileNet v1 model, based on original MobileNet v1 source model. 
...
def mobilenet_v1(inputs,
  ...
  network_fn = nets_factory.get_network_fn(isp_model_name, num_classes=num_classes, weight_decay=0.0, batch_norm_decay=0.95, 
        is_training=is_training)

  clean_image, isp_end_points = network_fn(inputs, alpha=alpha, sigma=sigma, bayer_mask=bayer_mask,
                                           use_anscombe=use_anscombe, noise_channel=noise_channel,
                                           num_layers=num_layers, num_iters=num_iters, is_real_data=is_real_data)
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
                        is_training=is_training):
      net, mobilenet_end_points = mobilenet_v1_base(clean_image, scope=scope,
                                          min_depth=min_depth,
